...
arg_definitions = Dict(
  "--plant_config" => ("Path to the plant configuration file", String, "config/plant-smd.yaml"),
  "--simulation_config" => ("Path to the simulation configuration file", String, "config/simulation-smd.yaml"),
  "--esn_config" => ("Path to the ESN configuration file", String, "config/esn-smd.yaml"),
  "--mpc_config" => ("Path to the MPC configuration file", String, "config/mpc-smd.yaml"),
...
mpc_params = Utils.load_hyperparameters(args["mpc_config"])
...
YAML.write_file(joinpath(config_dir, "plant-smd.yaml"), plant_params)
YAML.write_file(joinpath(config_dir, "simulation-smd.yaml"), sim_params)
YAML.write_file(joinpath(config_dir, "esn-smd.yaml"), esn_params)
YAML.write_file(joinpath(config_dir, "mpc-smd.yaml"), mpc_params)
...
if sim_params["PRETRAIN"]
  ...
  train_input = 0.5 .* mpc_params["INPUT_LIM"] .* (2 .* rand(sim_rng, train_step, plant_params["M"]) .- 1)
...
mpc_input_log = zeros(sim_step, plant_params["M"])
...
plant_x = plant_x0
prev_input = zeros(plant_params["M"], mpc_params["HORIZON"])
...
for t in 1:sim_step
  # 100ステップごとに符号を切り替え
  sign = (-1)^div(t - 1, 200)
  reference_outputs[t] = mpc_params["REF_OUT"] * sign
end

## MPC Simulation Loop
println("Start MPC Simulation.")
@showprogress for ind_ in 1:sim_step
  ...
  # --- MPC.controller_calc_inputの計算時間を測定 ---
  t_start = time()
  optimal_input, optimal_input_sequence, prev_input = MPC.controller_calc_input(mpc_params, prev_input, esn_x, ref_out, W, Win, Wout, ACT, leakrate)
  t_end = time()
  ...
  # ログ記録（この後でもOK）
  mpc_input_log[ind_, :] = optimal_input
...
println("End MPC Simulation.")


## Plot MPPI results
if sim_params["plot"]
  println("Plotting MPC results.")
  ...
  savefig(joinpath(fig_dir, "mpc_output.png"))
  ...
  plot(mpc_input_log, label="", xlabel="Time Step", ylabel="Control Input", title="MPPI Input Response", grid=true)
  savefig(joinpath(fig_dir, "mpc_input.png"))
  ...
  println("Plotting MPC results done.")
end


cost_output = MPC.calc_cost(mpc_params, plant_output_log; reference_outputs=reference_outputs)
cost_total = MPC.calc_cost_tot(mpc_params, plant_output_log, mpc_input_log; reference_input=ref_input, reference_outputs=reference_outputs)


println("mpc cost: ", cost_total)
