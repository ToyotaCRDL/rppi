...
arg_definitions = Dict(
  ...
  "--mppi_config" => ("Path to the MPPI configuration file", String, "config/mppi-smd.yaml"),
  ...
  "--sample" => ("Sample size for MPPI", Int, 1000),
...
YAML.write_file(joinpath(config_dir, "mppi-smd.yaml"), mppi_params)
...
for t in 1:sim_step
  # 100ステップごとに符号を切り替え
  sign = (-1)^div(t - 1, 200)
  reference_outputs[t] = mppi_params["REF_OUT"] * sign
end
...
@showprogress for ind_ in 1:sim_step
  ...
  optimal_input, prev_input, _, mppi_w = MPPI.controller_calc_input(
    mppi_params, prev_input, esn_x, ref_out, epsilon, Wout, esn_P, plant_params, W, Win, ACT, leakrate, mppi_covar_rng=mppi_covar_rng
  )
  t_end = time()
