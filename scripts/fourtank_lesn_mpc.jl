...
arg_definitions = Dict(
  ...
  "--mpc_config" => ("Path to the MPC configuration file", String, "config/mpc-fourtank.yaml"),
...
YAML.write_file(joinpath(config_dir, "mpc-fourtank.yaml"), mpc_params)
...
# ref_out = mpc_params["REF_OUT"]
ref_input = mpc_params["REF_INPUT"]
...
for t in 1:sim_step
  # 100ステップごとに符号を切り替え
  sign = (-1)^div(t - 1, 150)
  reference_outputs[t] = mpc_params["REF_OUT"] + 1.0 * sign
end
...
@showprogress for ind_ in 1:sim_step
  ...
  t_start = time()
  optimal_input, optimal_input_sequence, prev_input = MPC.controller_calc_input(mpc_params, prev_input, esn_x, ref_out, W, Win, Wout, ACT, leakrate, reference_input=ref_input)
  t_end = time()
